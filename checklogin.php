 ...
 *		
 *   @modified:
 *      Nuwan   -   Connected the database using datalink class. 
 */
...
function login() {
    ...
    if ($count == 1) {
        ...
        $redirect = @$_GET['url'];
        if ($redirect == "/Dropbox/checklogin.php" || $redirect == "" ) {
            $redirect = "/Dropbox/home.php";
        }
        ...
        echo header("location:/Dropbox/main_login.php?invalid=");
    }
}
...
if ($isLoggedin && $isPosting) {
    ...
    //session_start();
    //login();
    echo header("location:/Dropbox/main_login.php?logout=");
    ...
    echo header("location:/Dropbox/main_login.php?return=&url=" . $_SERVER['PHP_SELF']);
}

function getUserDetails($myusername) {
    $db_name = "project"; // Database name 
    $rm = new RolesManager($myusername);
    $t = $rm->getRoleCode();
    //$con = mysqli_connect("$host", "$username", "$password", "$db_name")or die("cannot connect");
	$obj=new DataLink("empdetail");
	$con=$obj->getConnection();
    // Check connection  
